...
/* Version ammended to use a MAX31855 SPI thermocouple sensor as input.
   This runs on a 3V3 SPI bus so should integrate with a ATmega 328 Low Power microcontroller
   The I2C bus will also support logic at this level.
...
//include <FatLib.h> // Not the stock Arduino SD library, use https://github.com/greiman/SdFat
...
//Employing a HD44780 driven display directly uses 6 pins which leave few for alternative
//uses. If you want to add any other indicators/LED/Encoder etc to this project, it is easiest to
//use a display with an I2C backpack - this is much easier to integrate and uses the SDA/SCL lines.

LiquidCrystal_I2C lcd(0x3F); //RS,EN,DB4,DB5,DB6,DB7
...
//These are NOT right!! Just for compilation testing
int maxSO = 10;
int maxSS0 = 11;
int maxSCK = 12;
Adafruit_MAX31855 kTC(maxSCK, maxSS0, maxSO);

DS1307RTC RTC; // define the Real Time Clock object
...
void loop(void)
{
        ...
        for (byte channel = 0; channel < chs; channel++) {
                ...
                kTC.readCelsius();
                delay(20);
                tempInArray[index][channel] = kTC.readCelsius();
